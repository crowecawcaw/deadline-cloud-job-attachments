...
from deadline.job_attachments._utils import (  # type: ignore[attr-defined]
...
# CONTROL BRANCH ONLY. Pre-fix _utils does not define this constant, so the probe
# supplies it in order to run against the old code at all. Without it the run would end
# in an ImportError, which proves nothing about path handling.
